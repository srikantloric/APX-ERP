...
function AddCasteCategory() {
    ...
    const handleSaveNewClass = async () => {
        ...
        if (!doc.exists) {
            await masterDataRef.set({ casteCategories: [newSectionData] });
        } else {
            await masterDataRef.update({
                casteCategories: firebase.firestore.FieldValue.arrayUnion(newSectionData)
            });
        }
        setNewSection("");
    ...
    useEffect(() => {
        const fetchClasses = () => {
            db.collection("MASTER_DATA").doc("masterData").get().then((doc) => {
                if (doc.exists) {
                    const data = doc.data();
                    setSections(data?.casteCategories.map((item: CasteCategoryType) => item));
                }
            });
        }
        fetchClasses();
