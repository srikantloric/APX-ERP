...
function AddReligions() {
    ...
    const handleSaveNewClass = async () => {
        ...
        if (!doc.exists) {
            await masterDataRef.set({ religions: [newReligionData] });
        } else {
            await masterDataRef.update({
                religions: firebase.firestore.FieldValue.arrayUnion(newReligionData)
            });
        }
        setNewReligion("");
    ...
    useEffect(() => {
        const fetchClasses = () => {
            db.collection("MASTER_DATA").doc("masterData").get().then((doc) => {
                if (doc.exists) {
                    const data = doc.data();
                    setReligions(data?.religions.map((item: ReligionType) => item));
                }
            });
        }
        fetchClasses();
