...
function AddSubjects() {
    ...
    const handleSaveNewClass = async () => {
        ...
        if (!doc.exists) {
            await masterDataRef.set({ subjects: [newSectionData] });
        } else {
            await masterDataRef.update({
                subjects: firebase.firestore.FieldValue.arrayUnion(newSectionData)
            });
        }
        setNewSection("");
        setSubjectRank("");
    ...
    useEffect(() => {
        const fetchClasses = () => {
            db.collection("MASTER_DATA").doc("masterData").get().then((doc) => {
                if (doc.exists) {
                    const data = doc.data();
                    setSections(data?.subjects.map((item: SubjectType) => item));
                }
            });
        }
        fetchClasses();
