...
const AdmissionEnquiry = () => {
  ...
  const onSubmit = async (data: EnquiryFormFields) => {
    ...
    db.collection("ADMISSION_ENQUIRY")
      ...
      .then(() => {
        ...
        enqueueSnackbar("Student Admission Enquiry Added :)", {
          variant: "success",
        });
      })
  ...
  ///Load Enquiries
  useEffect(() => {
    const admissionEnquiryListner = db
      .collection("ADMISSION_ENQUIRY")
      .orderBy("created_at", "desc")
      .onSnapshot((snapshot: any) => {
        if (snapshot.size) {
          const tempEnquiryArray: EnquiryFormFields[] = [];
          snapshot.forEach((document: any) => {
            const enquiryData = document.data() as EnquiryFormFields;
            enquiryData.doc_id = document.id;
            tempEnquiryArray.push(enquiryData);
            console.log(tempEnquiryArray);
          });
          setEnquiryData(tempEnquiryArray);
        }
      });

    return () => {
      if (admissionEnquiryListner) {
        admissionEnquiryListner();
      }
    };
  }, []);
  ...
  const deleteAdmision = async (docid: any) => {
    console.log(docid);
    return db
      .collection("ADMISSION_ENQUIRY")
      .doc(docid)
      .delete()
      .then(() => {
        enqueueSnackbar("Enquiry deleted from database", {
          variant: "success",
        });
      })
      .catch((e: any) => {
